...
from CopyExistencia import mouse_selection
...
def openFile():  # DIALOG TO CHOOSE A FILE
    excel_file = FileDialog.askopenfilename(
        initialdir=pathFile2,
        filetypes=(("xls", "*.xls"), ("all files", "*.*")),
        title="Escoja el archivo Excel a Procesar"
    )
...
def ordered_list(sheet, seleccion, option): # RECEIVE EXCEL SHIFT AND GENERETE A ORDERED LIST
    ...
    for i in range(sheet.nrows):
        ...
        if option == 1: # Buque import       
            if buque_import == seleccion:
              ...
              if linea == seleccion:
                    file_ordered = (f'Unidades en Existencia {linea} {format}.xlsx') # name of final file
                    #print (file_ordered)
                    if estatus == "EMT" or estatus == "TRV":
                        pies = int(float(pies))
                        ...
                        line = {"contenedor" : contenedor, "pies" : pies, "tipo": tipo, "iso" : iso, "linea" : linea, "estatus" : estatus,
                                   "ubicacion" : ubicacion, "fecha_ingreso" : fecha}
                        mtycontainers_linea.append(line)
                        #print (mtycontainers_linea)
    ...
    if len(mtycontainers_linea) > 0: # order list líneas        
        mtycontainers_linea.sort(key=lambda x: x.get("fecha_ingreso"), reverse=False)
...
def menu1():
    ornament()
    print ()
    print (" YARD MANAGEMENT ".center(espacios, '/'))
    ornament()
...
def menu2():
    ...
    print ("8. Ocupación")
    print ("9. Nueva Existencia")
    print ("10. Cambiar Existencia")
    print ("11. Contenedores por Tipo")
    print ("0. Salir")
    print ()
